/**
 * Set Mathematics full paper titles on pgallsemesters (sem2–sem4).
 * Run: node scripts/updateMathematicsSubjectFullNames.js
 */
...
} = require('../utils/mathematicsPaperNames');
...
const run = async () => {
  ...
  console.log('Updating Mathematics subject full titles...\n');

  const docs = await PGAllSemesters.find({ department: /math/i });

  console.log(`Mathematics students: ${docs.length}\n`);

  let studentsUpdated = 0;
  let subjectsUpdated = 0;

  for (const doc of docs) {
    if (!isMathematicsDepartment(doc.department)) continue;
    ...
    for (const { key, num } of SEM_KEYS) {
      ...
      block.subjects = block.subjects.map((sub) => {
        ...
        const updated = applyMathematicsFullNameToSubject(num, plain);
  ...
  const sample = await PGAllSemesters.findOne({ rollNo: 'MATH24-001' }).lean();
  if (sample) {
    console.log('\nSample MATH24-001:');
