...
const cleanupDuplicateCollections = async () => {
  try {
    ...
    const collectionsToCheck = [
      { old: 'ugfirstsem2025', new: 'ugfirstsem2025s' },
      { old: 'pgfirstsem2025', new: 'pgfirstsem2025s' }
    ];

    for (const { old: oldCollection, new: newCollection } of collectionsToCheck) {
      console.log(`\n📋 Checking ${oldCollection} and ${newCollection}...`);
      ...
      if (oldExists && newExists) {
        ...
        const newCount = await db.collection(newCollection).countDocuments();
        console.log(`   ✅ New collection exists with ${newCount} documents - no action needed`);
...
module.exports = { cleanupDuplicateCollections };
